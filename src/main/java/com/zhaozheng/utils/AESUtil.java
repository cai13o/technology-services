...
/*
   AES对报文进行加密，RSA对AES密钥加密，加密完的密钥密文追加到密文报文中

   解密：先取出AES密钥密文，用RSA解密得到AES密钥，再用AES对报文解密
*/
...
public class AESUtil {
    ...
    // 私钥
    // private static String AES_KEY;   // AES固定格式为128/192/256 bits.即：16/24/32bytes。DES固定格式为128bits，即8bytes。
     ...
     * 加密
     */
    public static String encrypt(String strJson, String Key) {
        ...
        try {
            ...
            byte[] encryptedData = cipher.doFinal(strJson.getBytes(CODE_TYPE));
    ...
    public static String aesDecrypt(String content, String Key) {
        // 解密
        String decryptResult = decrypt(content, Key);
        // 还原
        if(AES_TYPE.equals("AES/ECB/NoPadding")){
    ...
    public static String getAESKey() throws Exception {
        ...
        String encryptRandom = RSAUtil.rsaEncrypt(strRandom, RSAUtil.getKey(0));
        String strResult = "0123456789ABCDEF";
        if (encryptRandom.length() > 15){
            strResult = encryptRandom.substring(0, 16);
        } else {
            strResult = encryptRandom + strResult.substring(0, 16-encryptRandom.length());
        }
        return strResult;
    }
